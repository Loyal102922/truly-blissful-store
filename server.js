...
async function sendOwnerNewOrderEmail(order) {
  await transporter.sendMail({
    ...
    html: `
      <h2>🔥 NEW TRULY BLISSFUL ORDER</h2>
      ...
      <ul>${formatItems(order.cart)}</ul>

      <h3>Payment</h3>
      <p>
        <strong>Total Paid:</strong> $${order.total}<br>
        <strong>Status:</strong> ${order.status}<br>
        <strong>Stripe Session:</strong> ${order.stripeSessionId}
      </p>
    `
  });
}

async function sendCustomerOrderConfirmedEmail(order) {
  await transporter.sendMail({
    from: `"TRULY BLISSFUL" <${process.env.EMAIL_USER}>`,
    to: order.customerEmail,
    subject: "✅ Your TRULY BLISSFUL Order Is Confirmed",
    html: `
      ...
      <p>
        You will receive updates when your order is processing, shipped, tracking is added, and delivered.
      </p>

      <p>Thank you for supporting TRULY BLISSFUL.</p>
    `
  });
}
...
app.get('/order-details/:sessionId', async (req, res) => {
  try {
    ...
    await sendOwnerNewOrderEmail(order);
...
app.post('/track-behavior', async (req, res) => {
  try {
   ...
   await mongoClient.db("store").collection('behaviorAnalytics').insertOne({
...
app.get('/admin/behavior-analytics', async (req, res) => {
  try {
   const behavior = await mongoClient.db("store").collection('behaviorAnalytics').aggregate([
      {
        $group: {
          _id: '$productName',
          views: {
